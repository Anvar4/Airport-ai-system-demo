...
import { HiChevronDown, HiGlobeAlt, HiLogout, HiMenuAlt3, HiX } from 'react-icons/hi';
...
export default function Header({ activeTab }) {
  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled || activeTab !== 'home'
    ...
    >
      <nav className="container-custom flex items-center justify-between h-16 sm:h-20 px-4 sm:px-6 lg:px-8">
        <a href="#home" className="flex items-center gap-3 group translate-y-0 notranslate">
          <div className="w-10 h-10 rounded-xl bg-linear-to-br from-primary-500 to-accent-500 flex items-center justify-center shadow-lg shadow-primary-500/25 group-hover:shadow-primary-500/40 transition-shadow duration-300">
          ...
          {navLinks.map((link) => {
            ...
            return (
              <a
                key={link.href}
                href={link.href}
                className={`px-3 py-2 text-sm font-bold transition-all duration-200 rounded-lg ${isActive
        ...
        <div className="flex items-center gap-2 sm:gap-4 relative text-sm">
          <div className="relative" ref={langRef}>
            ...
            >
              ...
              <span className="font-bold relative -top-px">{activeLangObj.flag}</span>
          ...
          {user ? (
            <div className="relative" ref={profileRef}>
              ...
              >
                <div className="w-8 h-8 rounded-lg bg-linear-to-r from-primary-600 to-accent-600 flex items-center justify-center text-white font-bold text-sm notranslate">
            ...
            <div className="hidden sm:block">
              <a href="#auth" className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-linear-to-r from-primary-600 to-accent-600 font-bold text-white shadow-lg shadow-primary-500/20 hover:-translate-y-0.5 hover:shadow-primary-500/30 transition-all active:scale-95 leading-none">
                {t('nav.login')}
              </a>
            </div>
          )}
        ...
        {isMobileMenuOpen && (
          ...
          >
            <div className="container-custom px-4 py-4 flex flex-col gap-1 max-h-[70vh] overflow-y-auto">
              {navLinks.map((link) => {
                ...
                return (
                  <a
                    ...
                    className={`px-4 py-3 rounded-xl font-bold transition-all duration-200 ${isActive
              ...
              {!user && (
                <a href="#auth" onClick={handleNavClick} className="mt-2 text-center px-4 py-3 rounded-xl bg-linear-to-r from-primary-600 to-accent-600 font-bold text-white shadow-lg shadow-primary-500/20">
                  {t('nav.login')}
                </a>
              )}
            </div>
